...
local function do_teleport(sender)
    ...
    while impetus.getMedia() < 1000000 do
        if not has_printed then print("Low on media, please wait") end
        sleep(1)
    end

    local tele_pos = _G.destination
    if type(_G.destination) == "number" then
        tele_pos = get_stations()[_G.destination].position
    end

    focal_port.writeIota(tele_pos)
